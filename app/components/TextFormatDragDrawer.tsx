...
const TextFormatDragDrawer = ({
 ...
 const [alignment, setAlignment] = useState<AlignmentVariant, string>("left");
     ...
     <section className="flex items-center justify-between space-x-4">
      ...
      >
       {fontSizeList.map((fontSizeItem, id) => (
        <option key={id}>{fontSizeItem}</option>
       ))}
      </Select>
     </section>
    </div>
...
const ColorCircle = ({ color, fontColor, setFontColor }) => {
 return (
  <div
