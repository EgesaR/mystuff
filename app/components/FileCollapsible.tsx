...
const FileCollapsible: React.FC<FileCollapsibleProps> = ({
  ...
  return (
    <div className="w-full">
      <div className={`flex items-center w-full ${isCollapsed ? "justify-center" : ""} file-collapsible-header`}>
        {!isCollapsed && (
          <button
            type="button"
        ...
        <div className={`flex-1 flex items-center ${isCollapsed ? "justify-center" : "justify-between"}`}>
          <Link
            to={to || "#"}
            onClick={onClick}
            className="flex-1"
          ...
          >
            <div className={`w-full h-8 flex items-center ${isCollapsed ? "justify-center px-0" : "px-3"} rounded-md group hover:bg-zinc-800/40 transition-colors sidebar-btn`}>
              <div className={`flex items-center ${isCollapsed ? "" : "gap-3"}`}>
                <span className={`${color} w-5 h-5`}>{icon}</span>
                {!isCollapsed && <label className="text-sm font-medium text-white whitespace-nowrap sidebar-text">{text}</label>}
              </div>
            </div>
          </Link>
      ...
      <AnimatePresence>
        {isOpen && !isCollapsed && (
          <motion.div
            ...
            className="w-full overflow-hidden file-collapsible-content"
          >
